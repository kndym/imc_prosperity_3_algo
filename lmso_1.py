...
class Trader:
    def __init__(self):
        ...
        self.LIMIT = {"RAINFOREST_RESIN": 50, "filler": 50}
    ...
    def starfruit_fair_value(self, order_depth: OrderDepth, traderObject) -> float:
        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:
            ...
            filtered_ask = [
                ...
                >= self.params["filler"]["adverse_volume"]
            ]
            filtered_bid = [
                price
            ...
            if traderObject.get("starfruit_last_price", None) != None:
                ...
                pred_returns = (
                    last_returns * self.params["filler"]["reversion_beta"]
                )
    ...
    def run(self, state: TradingState):
        ...
        if "RAINFOREST_RESIN" in self.params and "RAINFOREST_RESIN" in state.order_depths:
            amethyst_position = (
            ...
            amethyst_take_orders, buy_order_volume, sell_order_volume = (
                self.take_orders(
                    ...
                    self.params["RAINFOREST_RESIN"]["fair_value"],
                    self.params["RAINFOREST_RESIN"]["take_width"],
                    amethyst_position,
                )
            )
            amethyst_clear_orders, buy_order_volume, sell_order_volume = (
                self.clear_orders(
                    ...
                    state.order_depths["RAINFOREST_RESIN"],
                    self.params["RAINFOREST_RESIN"]["fair_value"],
                    self.params["RAINFOREST_RESIN"]["clear_width"],
                    amethyst_position,
                    buy_order_volume,
                    sell_order_volume,
                )
        ...
        if "filler" in self.params and "filler" in state.order_depths:
            starfruit_position = (
                state.position["filler"]
                if "filler" in state.position
                else 0
            )
            starfruit_fair_value = self.starfruit_fair_value(
                state.order_depths["filler"], traderObject
            )
            starfruit_take_orders, buy_order_volume, sell_order_volume = (
                self.take_orders(
                    "filler",
                    state.order_depths["filler"],
                    starfruit_fair_value,
                    self.params["filler"]["take_width"],
                    starfruit_position,
                    self.params["filler"]["prevent_adverse"],
                    self.params["filler"]["adverse_volume"],
                )
            )
            starfruit_clear_orders, buy_order_volume, sell_order_volume = (
                self.clear_orders(
                    ...
                    state.order_depths["filler"],
                    starfruit_fair_value,
                    self.params["filler"]["clear_width"],
                    starfruit_position,
                    buy_order_volume,
                    sell_order_volume,
                )
            ...
            starfruit_make_orders, _, _ = self.make_orders(
                ...
                buy_order_volume,
                sell_order_volume,
                self.params["filler"]["disregard_edge"],
                self.params["filler"]["join_edge"],
                self.params["filler"]["default_edge"],
            )
            result["filler"] = (
        ...
        return result, conversions, "Hello"
